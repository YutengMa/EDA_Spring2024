...
epa_data <- read.csv(
  here("Processed_KEY","EPAair_O3_PM25_NC1819_Processed.csv"),
...
```{r data.summary, paged.print=FALSE}
...
```{r ozone2018, echo=FALSE, fig.align='left', fig.height= 3, warning=FALSE, fig.cap= "ozone2018",fig.show='hold'}

epa_data %>%
  filter(year(Date) == 2018) %>%
  ggplot(aes(x=factor(month(Date), label=month.abb), y=Ozone)) +
  geom_boxplot() +
  labs(x="Month", y="Ozone Level", title="Ozone levels by month in 2018")
```
```{r ozone2019, echo=FALSE, fig.align='left', fig.height= 3, warning=FALSE, fig.cap= "ozone2019", fig.show='hold'}
epa_data %>%
  filter(year(Date) == 2019) %>%
  ggplot(aes(x=factor(month(Date), label=month.abb), y=Ozone)) +
  geom_boxplot() +
  labs(x="Month", y="Ozone Level", title="Ozone levels by month in 2019")
```
...
# https://github.com/YutengMa/EDA_Spring2024/tree/main/Assignments